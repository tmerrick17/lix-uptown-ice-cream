...
class SinglePostCache extends \SmashBalloon\Reviews\Common\SinglePostCache
{
    ...
    private $provider_id;
    private $lang;
    private $media_finder;

    public function __construct($post_data, MediaFinder $media_finder, $provider_id = null)
    {
        ...
        $this->media_finder = $media_finder;
    }
        ...
        if (isset($this->post_data['review_id'])) {
            $feed_id_match = $wpdb->get_results($wpdb->prepare(
                "SELECT * FROM $table_name
                        WHERE post_id = %s AND lang = %s LIMIT 1", $this->post_data['review_id'], $this->lang
            ), ARRAY_A);
    ...
    public function media_finder_init()
    {
        $this->media_finder->set_post($this->post_data);
        $this->media_finder->set_provider($this->post_data['provider']['name']);
        $this->media_finder->construct_url_from_post_and_source();
        $media = $this->media_finder->search();
        $this->post_data['media'] = $media;
    }
}